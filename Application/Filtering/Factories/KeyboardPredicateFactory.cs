    ...
    public class KeyboardPredicateFactory : IPredicateFactory<Keyboard, KeyboardFilterModel>
    {
        ...
        private void AddTypeConstraint(ref Expression<Func<Keyboard, bool>> expression, ICollection<string> types)
        {
            if (types is not null && types.Any())
                expression = expression.And(k => types.Any(t => t.Equals(k.Type, StringComparison.InvariantCultureIgnoreCase)));
        }
        ...
        private void AddSizeConstraint(ref Expression<Func<Keyboard, bool>> expression, ICollection<string> sizes)
        {
            if (sizes is not null && sizes.Any())
                expression = expression.And(k => sizes.Any(s => s.Equals(k.Size, StringComparison.InvariantCultureIgnoreCase)));
        }
        ...
        private void AddKeyRolloverConstraint(ref Expression<Func<Keyboard, bool>> expression, ICollection<string> keyRollovers)
        {
        ...
        private void AddBacklightConstraint(ref Expression<Func<Keyboard, bool>> expression, ICollection<string> backlights)
        {
            if (backlights is not null && backlights.Any())
                expression = expression.And(k => backlights.Any(b => b.Equals(k.Backlight, StringComparison.InvariantCultureIgnoreCase)));
        }
    }
}