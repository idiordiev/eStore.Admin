...
namespace Infrastructure.Tests.Persistence
{
    ...
    public class KeyboardSwitchRepositoryTests
    {
        ...
        [TestCase(1, 3)]
        [TestCase(4, 1)]
        public async Task GetAllPagedAsync_ValidPagingParams_ReturnsRequiredKeyboardSwitches(int pageSize, int pageNumber)
        {
            // Arrange
        ...
        public async Task GetAllPaged_WithTracking_TracksChanges()
        {
            ...
            var keyboardSwitchToChange = keyboardSwitches.First(c => c.Id == 1);
            keyboardSwitchToChange.Name = changedName;
            
            // Assert
            Assert.That((await _context.KeyboardSwitches.FindAsync(1))?.Name, Is.EqualTo(changedName), "Changes has not been saved.");
        }
        ...
        [TestCase(1, 3)]
        [TestCase(4, 1)]
        public async Task GetByConditionPagedAsync_ValidPagingParamsAndCondition_ReturnsRequiredKeyboardSwitch(int pageSize, int pageNumber)
        {
            // Arrange
            var expected = _helper.KeyboardSwitches.Where(c => c.Id > 1).Skip(pageSize * (pageNumber - 1)).Take(pageSize);
            var pagingParams = new PagingParameters(pageSize, pageNumber);
        ...
        public async Task GetByConditionPagedAsync_WithTracking_TracksChanges()
        {
            ...
            // Act
            var keyboardSwitches = await _repository.GetByConditionPagedAsync(c => c.Id == 1, pagingParams, true, CancellationToken.None);
            var keyboardSwitchToChange = keyboardSwitches.First(c => c.Id == 1);
            keyboardSwitchToChange.Name = changedName;
        ...
        {
            // Arrange
            var expected = _helper.KeyboardSwitches.First(c => c.Id == id);
        ...
        public async Task GetByIdAsync_ExistingKeyboardSwitchWithTracking_ReturnsKeyboardSwitchAndTracksChanges(int id)
        {
            ...
            // Act
            var keyboardSwitch = await _repository.GetByIdAsync(id, true, CancellationToken.None);
            keyboardSwitch.Name = changedName;

            // Assert
            Assert.That((await _context.KeyboardSwitches.FindAsync(id))?.Name, Is.EqualTo(changedName), "Changes has not been tracked.");
        }
        ...
        public Task Add_NewKeyboardSwitch_AddsKeyboardSwitchToContext()
        {
            ...
            // Assert
            Assert.That(_context.KeyboardSwitches.Count(), Is.EqualTo(6), "The keyboard switch has not been added to context.");
        ...
        {
            // Arrange
            var keyboardSwitchToUpdate = _helper.KeyboardSwitches.First(c => c.Id == 1);
            const string changedName = "changedName";
            ...
            // Assert
            Assert.That(_context.KeyboardSwitches.Find(1)?.Name, Is.EqualTo(changedName), "The keyboard switch has not been updated.");
            return Task.CompletedTask;
        }
        ...
        {
            // Arrange
            var keyboardSwitchToDelete = _helper.KeyboardSwitches.First(c => c.Id == 1);
            ...
            // Assert
            Assert.That(await _context.KeyboardSwitches.FindAsync(1), Is.Null, "The keyboard switch has not been deleted.");
        }
    }
}